...
function setup(){
    ...
    laser4Group = new Group()
    laser5Group = new Group()
    laser6Group = new Group()
    laserFalling1Group = new Group()
    laserFalling2Group = new Group()
    laserFalling3Group = new Group()
    laserFalling4Group = new Group()
...
text("Hold 'S' to shoot (Dont move while shooting)",300,750)
 ...
 if(frameCount %100 === 0){
   ...
   laserFalling1Group.add(laserFalling1)

   var laserFalling2 = createSprite(displayWidth-800,random(20,20),10,200)
   laserFalling2.velocityY = +5
   laserFalling2.shapeColor = "red"
   laserFalling2Group.add(laserFalling2)

   var laserFalling3 = createSprite(displayWidth-600,random(20,20),10,200)
   laserFalling3.velocityY = +5
   laserFalling3.shapeColor = "red"
   laserFalling3Group.add(laserFalling3)

   var laserFalling4 = createSprite(displayWidth-400,random(20,20),10,200)
   laserFalling4.velocityY = +5
   laserFalling4.shapeColor = "red"
   laserFalling4Group.add(laserFalling4)

   Boss.depth = laser1.depth 
   Boss.depth = laser2.depth
   Boss.depth = laser3.depth
   Boss.depth = laser4.depth
   Boss.depth = laser5.depth
   Boss.depth = laser6.depth
   Boss.depth = Boss.depth +1

if(laser1Group.isTouching(cubert) || laserTopGroup.isTouching(cubert) || laser2Group.isTouching(cubert) || laser3Group.isTouching(cubert) || laser4Group.isTouching(cubert) || laser5Group.isTouching(cubert) || laser6Group.isTouching(cubert) || laserFalling1Group.isTouching(cubert) ||  laserFalling2Group.isTouching(cubert) ||  laserFalling3Group.isTouching(cubert) ||  laserFalling4Group.isTouching(cubert)){
score = score-1
}
